...
**Solução:** Use o subdomínio dedicado (SST) com seu próprio domínio. Requisições para `sst.seudominio.com.br` não são bloqueadas por bloqueadores de anúncio e IOS pois aparecem como requisições de primeira parte.
...
<Step title="Pixel instalado">Extensão Pixel Helper mostra badge verde no seu site. `window.mtrt` retorna `"function"` no console.</Step>
...
<Step title="UTMs capturados">Acesse seu site com parâmetros UTM. Verifique cookies/localStorage com os valores UTM armazenados.</Step>
...
<Step title="Painel mostrando dados">Eventos aparecem no painel do Metrito com atribuição de campanha e dados de lead corretos.</Step>
